      ...
      <div className="flex-1 overflow-auto p-4">
        {activeTab === 'overview' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">State</h3><p className="text-2xl">{stats.state||'stopped'}</p></div>
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">FPS</h3><p className="text-2xl">{(stats.fps||0).toFixed(1)}</p></div>
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Frames</h3><p className="text-2xl">{stats.frame_count||0}</p></div>
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Delta Time</h3><p className="text-2xl">{((stats.delta_time||0)*1000).toFixed(2)}ms</p></div>
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Layers</h3><p className="text-2xl">{stats.update_layers||0}</p></div>
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Callbacks</h3><p className="text-2xl">{stats.total_callbacks||0}</p></div>
            </div>
          </div>
        )}
        {activeTab === 'controls' && (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button className="px-4 py-2 bg-blue-600 text-white rounded text-sm" onClick={() => handleSubmit(`${API_BASE}/game-loop/resume`, {})}>Resume</button>
        ...
        {activeTab === 'timing' && (
          <div className="space-y-4">
            ...
            {frameTiming && <pre className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a] text-xs overflow-auto">{JSON.stringify(frameTiming, null, 2)}</pre>}
          </div>
        )}
      </div>
    </div>
