        ...
        {activeTab === 'overview' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Total Flows</h3><p className="text-2xl">{stats.total_flows ?? 0}</p></div>
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Total Artifacts</h3><p className="text-2xl">{stats.total_artifacts ?? 0}</p></div>
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Completed Flows</h3><p className="text-2xl">{stats.completed_flows ?? 0}</p></div>
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Active Flows</h3><p className="text-2xl">{stats.active_flows ?? 0}</p></div>
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Completion Rate</h3><p className="text-2xl">{stats.completion_rate != null ? (stats.completion_rate * 100).toFixed(1) + '%' : 'N/A'}</p></div>
            </div>
        ...
        {activeTab === 'flows' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={fetchFlows} className="px-3 py-1 bg-[#0f0f23] border border-[#2a2a4a] rounded text-xs text-gray-300 hover:bg-[#2a2a4a]">Refresh</button>
            </div>

            {flows.length > 0 ? (
              <div className="space-y-2">
                {flows.map((f: any, i: number) => (
                  <div key={f.id || i} className="bg-[#0f0f23] p-3 rounded border border-[#2a2a4a]">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-white text-sm font-medium">{f.name || f.id}</span>
                      <div className="flex gap-1">
                        <span className={`text-xs px-2 py-0.5 rounded ${f.status === 'completed' ? 'bg-green-900 text-green-300' : f.status === 'active' ? 'bg-blue-900 text-blue-300' : 'bg-gray-700 text-gray-300'}`}>{f.status || 'pending'}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 mt-2">
                      <button onClick={() => setSelectedFlow(f)} className="px-3 py-1 bg-[#1a1a2e] border border-[#2a2a4a] rounded text-xs text-gray-300 hover:bg-[#2a2a4a]">Details</button>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-gray-400 text-sm">No flows found</div>
            )}
            ...
            {selectedFlow && (
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a] mt-4">
                ...
                <pre className="text-xs text-gray-300 overflow-auto mb-3">{JSON.stringify(selectedFlow, null, 2)}</pre>
                <button onClick={() => setSelectedFlow(null)} className="px-3 py-1 bg-[#2a2a4a] text-gray-300 rounded text-xs hover:bg-[#3a3a5a]">Close</button>
              </div>
            )}
            ...
            <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a] mt-4">
              ...
              <div className="flex gap-3 items-end">
                <div className="flex-1">
                  <label className="text-xs text-gray-400 mb-1 block">Flow ID</label>
