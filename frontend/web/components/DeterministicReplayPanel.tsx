...
export default function DeterministicReplayPanel() {
  const [stats, setStats] = useState<SubsystemStats | null>(null);
  const [loading, setLoading] = useState(true);
  const [message, setMessage] = useState("");

  const [recordingName, setRecordingName] = useState("");
  const [recordingSeed, setRecordingSeed] = useState("42");
  const [playbackSessionId, setPlaybackSessionId] = useState("");

  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/deterministic-replay/stats`);
  ...
  const handleStartRecording = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/deterministic-replay/start-recording`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: recordingName,
          seed: parseInt(recordingSeed, 10),
        }),
      });
      const data = await res.json();
      if (data.error) {
  ...
  const handleStartPlayback = async () => {
    if (!playbackSessionId.trim()) {
      showMessage("Session ID is required");
      return;
    }
    try {
      const res = await fetch(`${API_BASE}/deterministic-replay/start-playback`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ session_id: playbackSessionId }),
      });
      const data = await res.json();
      if (data.error) {
        showMessage(`Error: ${data.error}`);
      } else {
        showMessage("Playback started successfully");
        ...
        <>
          ...
          <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  ...
                  <input
                    ...
                    placeholder="Seed"
                    value={recordingSeed}
                    onChange={(e) => setRecordingSeed(e.target.value)}
                    style={{
            ...
            <div style={{
              ...
              <h4 style={{ fontSize: "0.8rem", color: "#aaa", marginBottom: "0.5rem" }}>Start Playback</h4>
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                <input
                  type="text"
                  placeholder="Session ID"
                  value={playbackSessionId}
                  onChange={(e) => setPlaybackSessionId(e.target.value)}
                  style={{
                    flex: "1", padding: "0.5rem", borderRadius: "0.375rem",
                    border: "1px solid #2a2a4a", background: "#0d0d0d", color: "#e0e0e0",
                    fontSize: "0.8rem"
