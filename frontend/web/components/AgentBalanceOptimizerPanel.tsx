  ...
  const renderStatusTab = () => (
    <div>
      {data ? (
        ...
        <div className="text-gray-400 text-sm">No balance optimizer stats available</div>
      )}
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Parameter Type</label>
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Min Value</label>
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Max Value</label>
          ...
          <div className="col-span-2">
            <label className="text-xs text-gray-400 mb-1 block">Enumeration Options (comma-separated, for enum type)</label>
      ...
      <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-lg p-4">
        ...
        {filteredParams.length > 0 ? (
              ...
              <div key={p.id} className="bg-[#0f0f23] border border-[#2a2a4a] rounded-lg p-3">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex gap-1">
                    ...
                    <span className="text-xs bg-[#1a1a2e] text-gray-300 px-2 py-0.5 rounded">{p.domain}</span>
                  </div>
                </div>
          ...
          <div className="text-gray-400 text-xs">No parameters found</div>
        )}
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Tolerance</label>
    ...
    <div className="space-y-4">
      <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2">
            <label className="text-xs text-gray-400 mb-1 block">Parameter IDs (multi-select)</label>
            <div className="bg-[#0f0f23] border border-[#2a2a4a] rounded-lg p-2 max-h-32 overflow-auto">
              {parameters.length > 0 ? parameters.map((p) => (
                <label key={p.id} className="flex items-center gap-2 px-2 py-1 hover:bg-[#1a1a2e] rounded cursor-pointer">
                  ...
                  <span className="text-gray-500 text-xs">({p.domain})</span>
                </label>
              )) : (
                <div className="text-gray-500 text-xs px-2 py-1">No parameters available. Create some first.</div>
              )}
            </div>
          </div>
          <div className="col-span-2">
            <label className="text-xs text-gray-400 mb-1 block">Target IDs</label>
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Max Iterations</label>
      ...
      <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-lg p-4">
        ...
        {sessions.length > 0 ? (
              ...
              <div key={s.id} className="bg-[#0f0f23] border border-[#2a2a4a] rounded-lg p-3">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex gap-1">
                    ...
                    <span className="text-xs bg-[#1a1a2e] text-gray-300 px-2 py-0.5 rounded">{s.domain}</span>
                  </div>
                </div>
                <div className="flex items-center gap-4 text-xs text-gray-400">
          ...
          <div className="text-gray-400 text-xs">No sessions found</div>
        )}
      </div>
    </div>
  );
      ...
      {optResult && (
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-lg p-4">
              <span className="text-gray-400 text-xs">Best Fitness</span>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-[#2a2a4a]">
                      <th className="text-left p-2 text-gray-400">Iteration</th>
                      <th className="text-left p-2 text-gray-400">Fitness</th>
                      <th className="text-left p-2 text-gray-400">Avg Fitness</th>
                      {optResult.history[0] && Object.keys(optResult.history[0]).filter((k) => !['iteration', 'fitness', 'avg_fitness'].includes(k)).map((k) => (
                        <th key={k} className="text-left p-2 text-gray-400">{k.replace(/_/g, ' ')}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {optResult.history.map((h: IterationHistory, i: number) => (
                      <tr key={i} className="border-b border-[#2a2a4a]/30">
                        ...
                        {Object.entries(h).filter(([k]) => !['iteration', 'fitness', 'avg_fitness'].includes(k)).map(([k, v]) => (
                          <td key={k} className="p-2 text-gray-300 font-mono">{String(v)}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
      ...
      {reportData && (
        <div className="space-y-3">
          ...
          {reportData.before_metrics && reportData.after_metrics && (
            <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-lg p-4">
              <div className="text-sm font-medium text-gray-300 mb-3">Before / After Comparison</div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-[#2a2a4a]">
                    ...
                    {Object.keys(reportData.before_metrics).map((key) => {
                      ...
                      return (
                        <tr key={key} className="border-b border-[#2a2a4a]/30">
                          <td className="p-2 text-gray-300 capitalize">{key.replace(/_/g, ' ')}</td>
                          ...
                          <td className={`p-2 font-mono ${change > 0 ? 'text-green-400' : change < 0 ? 'text-red-400' : 'text-gray-400'}`}>
          ...
          {reportData.parameter_changes && reportData.parameter_changes.length > 0 && (
            <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-lg p-4">
              <div className="text-sm font-medium text-gray-300 mb-3">Parameter Changes</div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-[#2a2a4a]">
                      <th className="text-left p-2 text-gray-400">Parameter</th>
                      <th className="text-left p-2 text-gray-400">Before</th>
                      <th className="text-left p-2 text-gray-400">After</th>
                      <th className="text-left p-2 text-gray-400">Delta</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reportData.parameter_changes.map((pc, i) => (
                      <tr key={i} className="border-b border-[#2a2a4a]/30">
                        ...
                        <td className={`p-2 font-mono ${pc.after > pc.before ? 'text-green-400' : pc.after < pc.before ? 'text-red-400' : 'text-gray-400'}`}>
