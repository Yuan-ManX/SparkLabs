...
export default function EcosystemHubPanel() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/ecosystem-hub/stats`);
  ...
  const handleRegisterService = async () => {
    if (!serviceName.trim() || !serviceEndpoint.trim()) {
      showMessage("Name and endpoint are required");
      return;
    }
    try {
      const res = await fetch(`${API_BASE}/ecosystem-hub/register-service`, {
        method: "POST",
      ...
      showMessage("Failed to register service");
    }
  };

  const handleDiscoverServices = async () => {
    try {
      const res = await fetch(`${API_BASE}/ecosystem-hub/discover-services`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
      });
      ...
      if (data.error) {
        ...
        showMessage("Service discovery completed");
      }
    } catch {
      showMessage("Failed to discover services");
    }
  };
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h4 style={{ fontSize: "0.8rem", color: "#aaa", marginBottom: "0.5rem" }}>Register Service</h4>
            ...
            <div style={{
              ...
              <h4 style={{ fontSize: "0.8rem", color: "#aaa", marginBottom: "0.5rem" }}>Discover Services</h4>
              <button
