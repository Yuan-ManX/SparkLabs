      ...
      <div className="flex-1 overflow-auto p-4">
        {activeTab === 'overview' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] text-sm">Active</h3><p className="text-2xl">{stats.active_agents||0}</p></div>
            </div>
            {stats.mood_distribution && <div className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a]"><h3 className="text-[#00d4ff] mb-2">Mood Distribution</h3>{Object.entries(stats.mood_distribution).map(([k,v]) => <div key={k} className="flex justify-between text-sm"><span>{k}</span><span className="text-[#00d4ff]">{v as number}</span></div>)}</div>}
          </div>
        )}
        {activeTab === 'register' && (
          <div className="space-y-4">
        ...
        {activeTab === 'state' && (
          <div className="space-y-4">
            ...
            {agentState && <pre className="bg-[#0f0f23] p-4 rounded border border-[#2a2a4a] text-xs overflow-auto">{JSON.stringify(agentState, null, 2)}</pre>}
          </div>
        )}
      </div>
    </div>
