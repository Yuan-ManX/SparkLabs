...
class VisualScriptRuntime:
    ...
    def transpile_graph(self, graph_id: str) -> str:
        ...
        elif language == TargetLanguage.SPARKSCRIPT:
            code = self._transpile_sparkscript(graph)
    ...
    def _transpile_sparkscript(self, graph: ScriptGraph) -> str:
        """Transpile graph nodes to SparkScript source code."""
        lines: List[str] = []
        lines.append("# Generated by SparkLabs Visual Script Runtime")
        lines.append("")
        ...
        for event_node in event_nodes:
            ...
            for param in event_node.params:
                lines.append(f"{self.DEFAULT_INDENT}var {param.name} = {self._format_sparkscript_value(param.value)}")
    ...
    def _format_sparkscript_value(self, value: Any) -> str:
        """Format a value for SparkScript code output."""
        if isinstance(value, bool):
            return "true" if value else "false"
        if isinstance(value, (int, float)):
            return str(value)
